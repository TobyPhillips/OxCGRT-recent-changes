...
OxCGRTCountriesChange <- OxCGRTLastFortnight %>% group_by(CountryName) %>% summarise(SI_Changes = n_distinct(GovernmentResponseIndex, na.rm=T)) %>% filter(SI_Changes>1) %>% select(CountryName) %>% pull()
...
for (i in unique(Decreasing$CountryName)){
  ...
  i," (GRI: ",Decreasing$First[Decreasing$CountryName==i]," \U2191 ", Decreasing$Last[Decreasing$CountryName==i],")","</a> \n",sep ="")
  cat("</div>")}
cat("</div> <br/>")

NeutralChange <- filter(OxCGRT_GRIChange, MapColours==0)
cat("<h4>Neutral Change</h4>")
cat("<div class=\"contents\">")
for (i in unique(NeutralChange$CountryName)){
  ...
  i," (GRI: ",NeutralChange$First[NeutralChange$CountryName==i]," \U2191 ", NeutralChange$Last[NeutralChange$CountryName==i],")","</a> \n",sep ="")
  cat("</div>")}
cat("</div>")
