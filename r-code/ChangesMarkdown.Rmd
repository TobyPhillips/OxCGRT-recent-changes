...
OxCGRTCountriesChange <- OxCGRTLastFortnight %>% group_by(CountryName) %>% summarise(GRI_Changes = n_distinct(GovernmentResponseIndex, na.rm=T)) %>% filter(GRI_Changes>1) %>% select(CountryName) %>% pull()

# OxCGRT_GRIRanges <- OxCGRTLastFortnight %>% filter(CountryName %in% unique(ChangesTable$CountryName)) %>% group_by(CountryName) %>%
#   summarise(GRIMaxWeeklyChange = range(GovernmentResponseIndex, na.rm=T)[2]-range(GovernmentResponseIndex, na.rm=T)[1])

OxCGRT_GRIChange <- OxCGRTLastFortnight %>% filter(CountryName %in% unique(ChangesTable$CountryName)) %>% group_by(CountryName, CountryCode) %>% arrange(Date) %>%
  ...
  summarise(NumberChanges = length(GovernmentResponseIndex)-1,
    ...
    First=round(first(GovernmentResponseIndex),digits = 0), Last=round(last(GovernmentResponseIndex),digits = 0)) %>%
...
The Government Response Index is an aggregate summary of all 20 of our policy indicators. The index goes from 0 to 100, and a higher number generally means a more active policy (eg. more stringent restrictions, more generous economic support, or more pro-active public health measures). The following map shows countries that have changed their response at some point during the last two weeks, resulting in an increase or decrease in the overall Government Response Index. For more detail on the Government Response Index please see our most recent [working paper](https://www.bsg.ox.ac.uk/research/publications/variation-government-responses-covid-19) or explore our [dataset publication](https://doi.org/10.1038/s41562-021-01079-8).

```{r changes-map,echo=FALSE,include=F}
...
GRIChangeMapData <- left_join(worldMap, OxCGRT_GRIChange, by=c("iso_a3"="CountryCode"))
...
FillScale <- scale_fill_manual(name = "Government Response Index Change",values = IndexColours, limits = factor(c(1,0,2)),
                                         labels = c("Decreasing","No Change","Increasing"), na.value="grey90")

# The plot
png(filename =  "../images/changes-map.png", width = 700, height = 300)
ggplot(GRIChangeMapData, aes(geometry=geometry)) + geom_sf(aes(fill=MapColours), size=0.1)+
  theme(
    ...
    legend.spacing.y = unit(4, units = "pt"),
    legend.title = element_text(size=10, face = "bold"),
    legend.text = element_text(size=7),
    plot.margin=unit(c(0,0,0,0),"mm"),
    legend.position="bottom"
    ) + FillScale
# ggsave(file="../images/changes-map.png", width = 700, height = 300, units = "px", dpi = 300)
dev.off()
...
cat("<img src=\"../images/changes-map.png\">")
...
cat("<p style=\"text-align:right\"><i>")
cat("Last updated ", as.character(Sys.Date()))
cat("</i></p>")

Increasing <- filter(OxCGRT_GRIChange, MapColours==2)
cat("<h4>Overall increase</h4>")
cat("<div class=\"contents\">")
for (i in unique(Increasing$CountryName)){
  cat("<div>")
  cat("<a href=\"#",
  tolower(i),"-gri-",Increasing$First[Increasing$CountryName==i],
  "--",Increasing$Last[Increasing$CountryName==i],"\">",
  i," (GRI: ",Increasing$First[Increasing$CountryName==i]," \U2191 ", Increasing$Last[Increasing$CountryName==i],")","</a> \n",sep ="")
  cat("</div>")}
cat("</div> <br/>")

Decreasing <- filter(OxCGRT_GRIChange, MapColours==1)
cat("<h4>Overall decrease</h4>")
cat("<div class=\"contents\">")
for (i in unique(Decreasing$CountryName)){
  cat("<div>")
  cat("<a href=\"#",
  tolower(i),"-gri-",Decreasing$First[Decreasing$CountryName==i],
  "--",Decreasing$Last[Decreasing$CountryName==i],"\">",
  i," (GRI: ",Decreasing$First[Decreasing$CountryName==i]," \U2193 ", Decreasing$Last[Decreasing$CountryName==i],")","</a> \n",sep ="")
  cat("</div>")}
cat("</div> <br/>")

##### This would be great to do in theory, but not possible with current specification of changes list (which is based on index change, not policy change)
# NeutralChange <- filter(OxCGRT_GRIChange, MapColours==0)
# (OxCGRT_GRIChange$GRIChange == 0 & NeutralChanges$CountryName %in% OxCGRTCountriesChange$CountryName)
# if(length(NeutralChanges$CountryName)>0){
#   cat("<h4>No change in index (but a change in policy)</h4>")
#   cat("<div class=\"contents\">")
#   for (i in unique(NeutralChange$CountryName)){
#     cat("<div>")
#     cat("<a href=\"#",
#     tolower(i),"-gri-",NeutralChange$First[NeutralChange$CountryName==i],
#     "--",NeutralChange$Last[NeutralChange$CountryName==i],"\">",
#     i," (GRI: ",NeutralChange$First[NeutralChange$CountryName==i],")","</a> \n",sep ="")
#     cat("</div>")}
#   cat("</div>")
# }
...
FirstLast <- OxCGRTLastFortnight %>% filter(CountryName %in% OxCGRTCountriesChange) %>% group_by(CountryName, CountryCode) %>% arrange(Date) %>%
  select(CountryName,GovernmentResponseIndex) %>%
  distinct() %>% arrange(CountryName) %>% na.omit() %>%
  summarise(First=round(first(GovernmentResponseIndex),digits = 0), Last=round(last(GovernmentResponseIndex),digits = 0)) %>% select(CountryName,First,Last)
...
for (i in unique(FirstLast$CountryName)) {
  ...
  for (j in unique(CountrySubsetTemp$IndicatorFormatted)){
    ...
    cat(kableExtra::kbl(TD2,"html", col.names = NULL,escape = F) %>%
          ...
          column_spec(column=2, color = "grey80", width_min = "1in") %>%
...
This website and all OxCGRT data is published under a [Creative Commons CC BY 4.0 license](https://creativecommons.org/licenses/by/4.0/), meaning you are free to use it for any purpose, _provided you attribute us as the source_. You can read more about citing our work on our [GitHub repository](https://github.com/OxCGRT/covid-policy-tracker/blob/master/README.md#citing-our-data).

<p style="text-align:right"><i>
This summary page was written by Andrew Wood and Toby Phillips.
</i></p>
